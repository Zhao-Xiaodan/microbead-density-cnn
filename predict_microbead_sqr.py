...
# 2. Function to crop images in a grid pattern - ENHANCED for high resolution images
def crop_center_grid(image_path, output_folder, grid_cols=8, grid_rows=4, crop_size=512, visualize=True):
    """
    Creates a grid of uniform crops from the image, with calculated overlap to ensure full coverage.
    Optimized for 3840×2160 images with 8×4 grid of 512×512 crops.

    Args:
        image_path: Path to the input image
        output_folder: Directory to save crops
        grid_cols: Number of columns in the grid (default 8 for 3840 pixel width)
        grid_rows: Number of rows in the grid (default 4 for 2160 pixel height)
        crop_size: Size of each crop in pixels (default 512)
        visualize: Whether to create visualization images

    Returns:
        List of crop paths, list of crop info dictionaries, visualization path
    """
    ...
    # Debug info
    print(f"DEBUG: Using grid_cols={grid_cols}, grid_rows={grid_rows}, crop_size={crop_size}")

    # Calculate horizontal overlap and offset
    h_total = grid_cols * crop_size
    h_start_offset = 0
    h_overlap = 0
    if h_total > width:
        # Need overlap to fit all columns
        h_overlap = (h_total - width) / (grid_cols - 1)
        h_overlap = int(np.ceil(h_overlap))  # Round up to ensure all crops fit
        print(f"  Calculated horizontal overlap: {h_overlap} pixels for {grid_cols} columns")
    else:
        # No overlap needed, calculate start offset to center the grid
        h_start_offset = (width - h_total) // 2
        print(f"  No horizontal overlap needed, using start offset: {h_start_offset} pixels")

    # Calculate vertical offset - no overlap needed if grid_rows * crop_size <= height
    v_total = grid_rows * crop_size
    v_start_offset = 0
    v_overlap = 0
    if v_total > height:
        # Need overlap to fit all rows
        v_overlap = (v_total - height) / (grid_rows - 1)
        v_overlap = int(np.ceil(v_overlap))  # Round up to ensure all crops fit
        print(f"  Calculated vertical overlap: {v_overlap} pixels for {grid_rows} rows")
    else:
        # No overlap needed, calculate start offset to center the grid
        v_start_offset = (height - v_total) // 2
        print(f"  No vertical overlap needed, using start offset: {v_start_offset} pixels")
    ...
    for col in range(grid_cols):
        if h_total > width:
            ...
            # Use offset
            x = h_start_offset + col * crop_size
    ...
    for row in range(grid_rows):
        if v_total > height:
            ...
            # Use offset
            y = v_start_offset + row * crop_size
    ...
    crop_paths = []
    crop_info = []  # Store detailed info for each crop

    # Calculate physical dimensions for density conversion
    # Assuming 1.5 μm per pixel (from 4x objective)
    um_per_pixel = 1.5
    physical_width_um = width * um_per_pixel
    physical_height_um = height * um_per_pixel
    crop_physical_width_um = crop_size * um_per_pixel
    crop_physical_height_um = crop_size * um_per_pixel
    crop_area_mm2 = (crop_physical_width_um * crop_physical_height_um) / 1000000  # Convert to mm²

    for row in range(grid_rows):
        for col in range(grid_cols):
            # Get coordinates for this crop
            x = x_positions[col]
            y = y_positions[row]

            # Check if crop extends beyond image bounds and adjust if needed
            x_end = min(x + crop_size, width)
            y_end = min(y + crop_size, height)

            # If the crop would be smaller than expected, skip it
            if x_end - x < crop_size or y_end - y < crop_size:
                print(f"  Warning: Crop at r{row}c{col} would be smaller than {crop_size}x{crop_size}, skipping")
                continue
            ...
            if crop.shape[0] != crop_size or crop.shape[1] != crop_size:
                print(f"  Warning: Crop at r{row}c{col} has dimensions {crop.shape[0]}x{crop.shape[1]}, expected {crop_size}x{crop_size}")
                continue
            ...
            # Debug info about the crop
            print(f"  Creating crop r{row}c{col} at position ({x},{y}) with size {crop.shape[1]}x{crop.shape[0]}")

            # Try to write the crop and validate it was written correctly
            success = cv2.imwrite(crop_path, crop)
            if not success:
                print(f"  ERROR: Failed to write crop to {crop_path}")
                continue

            # Verify the crop was saved correctly
            if os.path.exists(crop_path):
                verify_img = cv2.imread(crop_path)
                if verify_img is None:
                    print(f"  ERROR: Failed to read back crop from {crop_path}")
                    continue
                elif verify_img.shape[0] != crop_size or verify_img.shape[1] != crop_size:
                    print(f"  ERROR: Saved crop has wrong dimensions: {verify_img.shape}, expected {crop_size}x{crop_size}")
                    # Try to fix the crop
                    try:
                        os.remove(crop_path)  # Remove the bad crop
                        # Re-create properly sized crop
                        resized_crop = cv2.resize(crop, (crop_size, crop_size), interpolation=cv2.INTER_AREA)
                        cv2.imwrite(crop_path, resized_crop)
                        print(f"  Fixed: Resized and saved crop at {crop_path}")
                    except Exception as resize_err:
                        print(f"  ERROR: Failed to fix crop dimensions: {resize_err}")
                        continue
            else:
                print(f"  ERROR: Crop file {crop_path} does not exist after writing")
                continue

            # Store crop path and position info with physical dimensions
            ...
            crop_info.append({
                ...
                "width": crop_size,
                "height": crop_size,
                "area_mm2": crop_area_mm2  # Store physical area for density calculation
            })

            # Draw rectangle for this crop on visualization image
            if visualize:
                color = (0, 255, 0)  # Green for all grid cells
    ...
    print(f"  Created {len(crop_paths)} crops from {image_path}")
...
def create_boxplot_with_scatter(results_df, output_path, title, combine_labels=False):
    ...
    for i, patch in enumerate(box.artists):
        if i < len(unique_labels):
            # Extract the base label without the index
            if '|' in unique_labels[i]:
                label = unique_labels[i].split('|')[0]
            else:
                label = unique_labels[i].split('_')[0]

            # Find color index for this group
            try:
    ...
    plt.ylabel('Predicted Density (beads/mm²)', fontsize=14)
...
# 6. Function to create heatmap visualization of predicted densities - ENHANCED for high-res images
def create_density_heatmap(image_name, predictions, grid_positions, grid_size, output_path, enhance_colors=True):
    """
    Create a heatmap visualization showing the predicted densities for each grid cell
    Enhanced for high-resolution microscopy images with 8×4 grid

    Args:
        image_name: Name of the image
        predictions: List of density predictions for each grid cell
        grid_positions: List of grid positions (row, col dictionaries) for each prediction
        grid_size: Size of the grid (rows)
        output_path: Path to save the output figure
        enhance_colors: Whether to enhance color contrast for better visualization
    """
    # Get grid dimensions from the data
    rows = grid_size  # Use the passed grid_size parameter as the number of rows
    cols = max([p['col'] for p in grid_positions]) + 1  # Calculate columns from data
    ...
    for position, density in zip(grid_positions, predictions):
        row = position['row']
        col = position['col']
    ...
    # Calculate the mean and standard deviation of non-NaN values
    valid_data = heatmap_data[~np.isnan(heatmap_data)]
    data_mean = np.mean(valid_data) if len(valid_data) > 0 else 0
    data_std = np.std(valid_data) if len(valid_data) > 0 else 0

    # Get magnification from image name for simplified title
    mag = "Unknown"
    if 'X_' in image_name or 'X ' in image_name or '_' in image_name:
        try:
            pattern = r'(\d+)[xX]'
    ...
    # Use a consistent colormap with improved contrast
    if enhance_colors:
        # Calculate vmin and vmax for enhanced color contrast
        # Show values from mean - 1.5*std to mean + 1.5*std
        vmin = max(0, data_mean - 1.5 * data_std)
        vmax = data_mean + 1.5 * data_std
        cmap = "YlGnBu_r"  # Reversed YlGnBu colormap for better contrast
    else:
        vmin = None
        vmax = None
        cmap = "YlGnBu"

    # Create heatmap with improved annotation
    ax = sns.heatmap(masked_data, annot=True, fmt=".1f", cmap=cmap,
                    vmin=vmin, vmax=vmax,
                    cbar_kws={'label': 'Predicted Density (beads/mm²)'})

    # Add statistics in the title
    stats_text = f"Mean: {data_mean:.1f}, Std: {data_std:.1f}, Min: {np.nanmin(heatmap_data):.1f}, Max: {np.nanmax(heatmap_data):.1f}"
    plt.title(f"Density Heatmap for {mag}\n{stats_text}", fontsize=16)
    plt.tight_layout()
    ...
    return output_path
# 7. Function to extract labels from CSV or Excel files in a subfolder - IMPROVED VERSION
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Predict microbead densities on high-resolution microscopy images')
    ...
    parser.add_argument('--create_heatmaps', action='store_true',
                        help='Create heatmap visualizations for each image (default: True)')
    ...
    parser.add_argument('--um_per_pixel', type=float, default=1.5,
                        help='Micrometers per pixel for the microscope (default: 1.5 for 4x objective)')
    parser.add_argument('--enhance_colors', action='store_true',
                        help='Enhance color contrast in heatmaps (default: False)')
    ...
    for subfolder in subfolders:
        ...
        # Find all images in this subfolder - use os.walk to handle special characters better
        ...
        for img_path in image_paths:
            ...
            # Crop the image into grid cells using the updated grid_cols and grid_rows parameters
            crop_paths, crop_info, viz_path = crop_center_grid(
                str(img_path), image_crops_dir, args.grid_cols, args.grid_rows, args.crop_size, True
            )
            ...
            predictions = []
            valid_positions = []
            for crop_path, position in zip(crop_paths, crop_info):
                density = predict_density(model, crop_path, transform, device)
                if density is not None:
                    # Convert to density per mm²
                    area_mm2 = position['area_mm2']
                    density_per_mm2 = density
                    predictions.append(density_per_mm2)
                    valid_positions.append(position)
                    print(f"    Crop at position r{position['row']}c{position['col']}: Predicted density = {density_per_mm2:.2f} beads/mm²")
            ...
            if args.create_heatmaps and predictions:
                ...
                create_density_heatmap(image_name, predictions, valid_positions,
                                     args.grid_rows, heatmap_path, args.enhance_colors)
                print(f"    Created heatmap at {heatmap_path}")

            # Clean up temporary crop directory if not saving crops
            if not args.save_crops and os.path.exists(image_crops_dir):
                try:
                    import shutil
        ...
        if subfolder_results:
            ...
            # Add total bead count estimation (density * area)
            summary_df['estimated_bead_count'] = summary_df['mean_density'] * summary_df['total_area_mm2']
    ...
    if not all_results.empty:
        ...
        # Add estimated bead count
        combined_summary_df['estimated_bead_count'] = combined_summary_df['mean_density'] * combined_summary_df['total_area_mm2']
        ...
        print(f"\nAverage predicted density across all images: {avg_density:.2f} beads/mm²")

        # Calculate total estimated bead count
        total_area_mm2 = all_results['area_mm2'].sum()
        total_bead_estimate = avg_density * total_area_mm2
        print(f"Total estimated bead count: {total_bead_estimate:.0f} beads (over {total_area_mm2:.2f} mm²)")
    else:
    ...
    config = {
        ...
        'total_area_mm2': float(total_area_mm2) if 'total_area_mm2' in locals() else None,
        'total_bead_estimate': float(total_bead_estimate) if 'total_bead_estimate' in locals() else None
    }
